...
uint8_t adc_updateHysteresis(uint16_t inputLevel,uint8_t adcNum) {
	...
	printf("input %d\n",inputLevel);
	#endif
	uint16_t prevLevelFull = ((uint16_t)adcValuesNew[adcNum] << HYST_WIDTH) | OFFSET;//
	#ifdef ECLIPSE_DEBUG
	printf("prevLevelFull  %d prevLevel %d HYSTWIDTH  %d    OFFSET  %d\n",prevLevelFull,adcValuesNew[adcNum],HYST_WIDTH,OFFSET);
	#endif
	uint16_t lowerbound = adcValuesNew[adcNum] > 0 ? prevLevelFull - MARGIN : 0;
	uint16_t upperbound = adcValuesNew[adcNum] < MAX_OUT ? prevLevelFull + MARGIN : MAX_IN;
	#ifdef ECLIPSE_DEBUG
	printf("lowerbound  %d upperbound %d MAX_OUT  %d    MAX_IN  %d MARGIN %d\n",lowerbound,upperbound,MAX_OUT,MAX_IN,MARGIN);
	#endif
	if (inputLevel < lowerbound || inputLevel > upperbound) {
		adcValuesNew[adcNum] = inputLevel >> HYST_WIDTH;
		#ifdef ECLIPSE_DEBUG
		printf("new level  %d\n",adcValuesNew[adcNum]);
		#endif
		return 1;
...
/**
    ...
    uint16_t adc_filter(uint16_t input,uint8_t adcNum) {
        filtered[adcNum] += input;
        uint16_t output = (filtered[adcNum] + fixedPointAHalf) >> K;
        filtered[adcNum] -= output;
        return output;
    }
